...
export function TestDataGenerator() {
  const { orders, dispatch } = useWorkforce();
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <div style={{
          ...
          <h2 style={{ fontSize: 'var(--fs-h3)', fontWeight: 700, marginBottom: 'var(--space-4)' }}>
            Generate Orders & Patient Messages
          </h2>
          ...
          {/* Service picker (only in service mode) */}
          {cfg.mode === 'service' && (
            <div style={{ marginBottom: 'var(--space-4)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                ...
                <input
                  ...
                  min={1} max={500}
                  value={cfg.count}
                  onChange={e => setCfg(c => ({ ...c, count: Math.min(500, Math.max(1, +e.target.value)) }))}
                  style={{ ...inputSty, width: 60 }}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <select
                value={cfg.urgency}
                onChange={e => setCfg(c => ({ ...c, urgency: e.target.value as Urgency | 'mixed' }))}
                style={inputSty}
              >
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input
                  type="number" min={0} max={168} value={cfg.ageMin}
                  onChange={e => setCfg(c => ({ ...c, ageMin: +e.target.value }))}
                  style={{ ...inputSty, width: 60 }}
                />
                <span style={{ color: 'var(--fg3)', fontSize: 'var(--fs-small)' }}>to</span>
                <input
                  type="number" min={0} max={168} value={cfg.ageMax}
                  onChange={e => setCfg(c => ({ ...c, ageMax: +e.target.value }))}
                  style={{ ...inputSty, width: 60 }}
                />
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: 'var(--fg3)', cursor: 'pointer', fontSize: 'var(--fs-micro)',
                    }}
                  >
                    {p.label}
                  </button>
          ...
          <div style={{
            ...
            {cfg.ageMax > 24 && (
              <div style={{ fontSize: 'var(--fs-micro)', color: 'var(--warning)' }}>
                ⚠ Orders older than 24h may already be in SLA warning territory depending on configuration.
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center' }}>
            <Button variant="primary" size="md" onClick={generate}>
              ⚗ Generate {cfg.count} orders
            </Button>
            <Button variant="ghost" size="md" onClick={clearGenerated}>
              Clear all generated
            </Button>
            {lastGenCount !== null && (
              <Badge variant="success" size="md">✓ {lastGenCount} orders added</Badge>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)', marginBottom: 'var(--space-3)' }}>
            ...
            <StatBox label="Pending / escalated" value={pendingOrders} color="var(--warning)" />
